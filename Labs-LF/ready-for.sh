...
#               2014-2016 Jan-Simon M̦ller <dl9pf@gmx.de>
#
# Licensed under GPL
#
...
# Further updates by Jan-Simon M̦ller
#
# Version 8.35: 2022-03-23 ready-for.sh
#   - Many package updates
#   - Splut @trace into @ktrace
#   - Add LFS200
#   - Remove LFC110
#   - Updates to LFD232, LFD254, LFD401, LFD430, LFD460, LFC110, LFS143, LFS157, LFS177, LFS201, LFS211, LFS241, LFS301, LFS311, LFS462, LFW111
#   - Add CentOS-9-Steeam, LinuxMint-20.2, LinuxMint-20.3
#   - Remove Debian-9, Fedora-33, LinuxMint-20.1, openSUSE-15.2
#   - Only cache success output
#   - Better wget handling
#   - Better handling of not enough disk space for a class
# 
#===============================================================================
VERSION=8.35
...
get() {
    ...
    local WGET="wget --quiet --no-cache --no-check-certificate --prefer-family=IPv4"
    if [[ -n $NONETWORK ]] ; then
...
check_time() {
    ...
    if [[ -n $NONETWORK ]] ; then
        ...
        OUTSIDE=$(wget -q --prefer-family=IPv4 -O- "http://worldtimeapi.org/api/ip" \
                | sed -e 's/^.*unixtime"://; s/,.*$//')
    elif command -v curl >/dev/null ; then
...
get_file() {
    ...
    if [[ -n $NONETWORK ]] ; then
        ...
        for URL in $URLS ; do
            ...
            # shellcheck disable=SC2086
            if $DRYRUN wget --prefer-family=IPv4 $WGET_CM_OPTS $OPTS "$URL" ; then
                break
            fi
        done
    elif command -v curl >/dev/null ; then
...
try_file() {
    ...
    for URL in $(get_urls "$ACTIVITY${FILE:+/$FILE}") ; do
        ...
        # shellcheck disable=SC2086
        SIZE=$(wget --prefer-family=IPv4 --server-response --spider --quiet $WGET_CM_OPTS "$URL" 2>&1 \
...
DISK=5
#DISTROS=
DISTROS[EMBEDDED]="ArchLinux AlmaLinux-8+ CentOS-8+ Debian-10+ Fedora-34+ LinuxMint-20+ openSUSE-15+ RockyLinux-8+ Ubuntu-20.04+LTS+"
DISTROS[LFD]="ArchLinux AlmaLinux-8+ CentOS-8+ Debian-10+ Fedora-34+ LinuxMint-20+ openSUSE-15+ RHEL-8+ RockyLinux-8+ Ubuntu-18.04+LTS+ SLES-15+"
DISTROS[LFS]="AlmaLinux-8+ CentOS-7+ Debian-10+ Fedora-34+ LinuxMint-20+ openSUSE-15+ RHEL-7+ RockyLinux-8+ Ubuntu-18.04+LTS+ SLES-15+"
DISTRO_ARCH=
DISTRO_BL="CentOS-6- Debian-9- Fedora-32- LinuxMint-19.2- openSUSE-42.3- OracleLinux RHEL-6- SLES-12.2- Ubuntu-21.04-"
DISTRO_BL[EMBEDDED]="CentOS-7- Debian-9- 5 Fedora-32- LinuxMint-19.2- openSUSE-42.3- OracleLinux RHEL SLES Ubuntu-18.04 Ubuntu-21.04-"
...
RECOMMENDS[CentOS.Stream_@build]="-ccache"
#-------------------------------------------------------------------------------
...
SUGGESTS[CentOS-7_@common]="[CentOS_@common] -gnome-tweaks gnome-tweak-tool"
SUGGESTS[CentOS-8_@common]="[CentOS_@common]"
RECOMMENDS[CentOS.Stream_@common]="-screen"
SUGGESTS[CentOS.Stream_@common]="-gparted"
...
PACKAGES[CentOS.Stream_@embedded]="-screen"
RECOMMENDS[CentOS-7_@embedded]="uboot-tools"
RECOMMENDS[CentOS-8_@embedded]="bmap-tools"
RECOMMENDS[CentOS.Stream_@embedded]="-bmap-tools -uboot-tools"
RECOMMENDS[Fedora_@embedded]="bmap-tools uboot-tools"
#-------------------------------------------------------------------------------
...
PACKAGES[CentOS.Stream_@kernel]="-indent"
PACKAGES[CentOS.Stream-9_@kernel]="-ctags -indent"
...
SUGGESTS[CentOS_@kernel]="gtk2-devel libglade2-devel slang-devel"
...
#-------------------------------------------------------------------------------
PACKAGES[CentOS_@perf]="binutils-devel libdwarf-devel elfutils-libelf-devel
	slang-devel openssl-devel libunwind-devel libzstd-devel zlib-devel"
PACKAGES[CentOS.Stream_@perf]="-libunwind-devel"
PACKAGES[CentOS.Stream-9_@perf]="-libdwarf-devel"
...
PACKAGES[CentOS.Stream_@stress]="-stress"
...
PACKAGES[CentOS-8_@sysadm]="[Fedora_@sysadm] -bonnie++ -collectl"
PACKAGES[CentOS.Stream_@sysadm]="[CentOS-8_@sysadm]"
...
PACKAGES[CentOS-7_@virt]="ebtables -edk2-ovmf -spice-gtk"
PACKAGES[CentOS-8_@virt]="-"
PACKAGES[CentOS.Stream-8_@virt]="-"
PACKAGES[CentOS.Stream-9_@virt]="-spice-gtk -spice-gtk-tools"
PACKAGES[Fedora_@virt]="spice-gtk"
PACKAGES[openSUSE_@virt]="ebtables libvirt-daemon qemu-kvm spice-gtk -spice-gtk-tools"
...
PACKAGES[CentOS_LFD301]="git-all expat-devel openssl-devel"
...
PACKAGES[CentOS_LFD401]="glibc-static libaio-devel"
PACKAGES[CentOS-7_LFD401]="ElectricFence"
PACKAGES[CentOS-8_LFD401]="-"
PACKAGES[CentOS.Stream_LFD401]="[CentOS_LF401] -ddd -ElectricFence"
PACKAGES[Fedora_LFD401]="[CentOS_LFD401] ElectricFence kcachegrind"
...
#-------------------------------------------------------------------------------
PACKAGES[LFD450]="@build @common @embedded @kernel @qemu @qemuriscv @ktrace @trace help2man rsync"
...
PACKAGES[Ubuntu_LFD460]="liblz4-tool libegl1-mesa libsdl2-dev mesa-common-dev
	pylint3 python3 python3-git python3-subunit zstd"
PACKAGES[Ubuntu-20.04_LFD460]="-pylint3 pylint"
PACKAGES[Debian-999_LFD460]="-pylint3"
...
PACKAGES[CentOS-7_LFD460]="-python3-jinja2 -python3-pexpect -python3-ply -rpcgen"
PACKAGES[CentOS-8_LFD460]="python3-GitPython"
PACKAGES[CentOS.Stream_LFD460]="-python3-GitPython"
...
PACKAGES[openSUSE_LFD460]="libSDL-devel Mesa-dri-devel
...
PACKAGES[CentOS_LFS176]="libsodium"
PACKAGES[CentOS.Stream_LFS176]="-libsodium"
...
DISK[LFS301]="1"
#-------------------------------------------------------------------------------
PACKAGES[LFS301]="@build @common @docker @sysadm @trace @virt
	cryptsetup lvm2 quota xfsprogs"
PACKAGES[Debian_LFS301]="btrfs-progs"
PACKAGES[CentOS-7_LFS301]="btrfs-progs -cryptsetup"
PACKAGES[CentOS-8_LFS301]="-"
PACKAGES[openSUSE_LFS301]="btrfsprogs"
...
#-------------------------------------------------------------------------------
PACKAGES[LFS426]="@stress @sysadm @ktrace @trace @virt blktrace blktrace crash
...
PACKAGES[CentOS_LFS426]="blas-devel kernel-devel libaio-devel
	ncurses-devel numactl-libs perl-Time-HiRes zlib-devel"
PACKAGES[CentOS-7_LFS426]="libhugetlbfs-utils oprofile"
PACKAGES[CentOS-8_LFS426]="libhugetlbfs-utils"
